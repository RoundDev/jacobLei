...
export class PaymentComponent implements OnInit, AfterViewInit {
  ...
  async createPayment(token) {
    ...
    let intAmount = 0;
    const amount = (<HTMLInputElement>document.getElementById('amountToPay')).value;
    const first_name = (<HTMLInputElement>document.getElementById('first_name')).value;
    const last_name = (<HTMLInputElement>document.getElementById('last_name')).value;
    const email_address = (<HTMLInputElement>document.getElementById('email_address')).value;
    const phone_number = (<HTMLInputElement>document.getElementById('phone_number')).value;
    intAmount = parseInt(amount, 10);
    this.sendSqPayment({'data': body, 'amountToPay': amount, first_name, last_name, email_address, phone_number});
